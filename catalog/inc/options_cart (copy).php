...
<?php
  ...
  if (isset($_POST['submitted'])) {
    ...
    $succ = (bool) $xml->saveXML(GSDATAOTHERPATH . $this->id . '/cart.xml');
...
<div class="leftsec">
  <p>
	  ...
	  <input type="checkbox" name="enabled" value="y" <?php if ($cart->getEnabled() == 'y') echo 'checked="checked"'; ?>>
	</p>
  <p>
	  <label for="labelname"><?php i18n($this->id . '/NAME'); ?> (<?php i18n($this->id . '/LABEL'); ?>) : </label>
	  <input type="text" class="text" name="labelname" value="<?php echo $cart->getLabelName(); ?>"/>
	</p>
	<p>
	  <label for="labelprice"><?php i18n($this->id . '/PRICE'); ?> (<?php i18n($this->id . '/LABEL'); ?>) : </label>
	  <input type="text" class="text" name="labelprice" value="<?php echo $cart->getLabelPrice(); ?>"/>
	</p>
	<p>
	  <label for="labelquantity"><?php i18n($this->id . '/QUANTITY'); ?> (<?php i18n($this->id . '/LABEL'); ?>) : </label>
	  <input type="text" class="text" name="labelquantity" value="<?php echo $cart->getLabelQuantity(); ?>"/>
	</p>
	<p>
	  <label for="labeltotal"><?php i18n($this->id . '/TOTAL'); ?> (<?php i18n($this->id . '/LABEL'); ?>) : </label>
	  <input type="text" class="text" name="labeltotal" value="<?php echo $cart->getLabelTotal(); ?>"/>
	</p>
	<p>
	  <label for="checkoutemail"><?php i18n($this->id . '/EMAIL'); ?> : </label>
	  <input type="text" class="text" name="checkoutemail" value="<?php echo $cart->getCheckoutEmail(); ?>"/>
	</p>
	<p>
	  <label for="currency"><?php i18n($this->id . '/CURRENCY'); ?> : </label>
	  <input type="text" class="text" name="currency" maxlength="3" value="<?php echo $cart->getCurrency(); ?>"/>
	</p>
	<p>
	  <label for="language"><?php i18n($this->id . '/LANGUAGE'); ?> : </label>
	  <input type="text" class="text" name="language" value="<?php echo $cart->getLanguage(); ?>"/>
	</p>
</div>
<div class="rightsec">
	<p>
	  <label for="taxrate"><?php i18n($this->id . '/TAX_RATE'); ?> : </label>
	  <input type="text" class="text" name="taxrate" value="<?php echo $cart->getTaxRate(); ?>"/>
	</p>
	<p>
	  <label for="taxshipping"><?php i18n($this->id . '/TAX_SHIPPING'); ?> : </label>
	  <input type="text" class="text" name="taxshipping" value="<?php echo $cart->getTaxShipping(); ?>"/>
	</p>
	<p>
	  <label for="cartystyle"><?php i18n($this->id . '/STYLE'); ?>: </label>
	  <select class="text" name="cartstyle">
      <option value="div" <?php if ($cart->getCartStyle() == 'div') echo 'selected'; ?>>
        div
      </option>
      <option value="table" <?php if ($cart->getCartStyle() == 'table') echo 'selected'; ?>>
        table
      </option>
    </select>
	</p>
	<p>
      ...
      <option value="PayPal" <?php if ($cart->getCheckoutType() == 'PayPal') echo 'selected'; ?>>
        <?php i18n($this->id . '/PAYPAL'); ?>
      </option>
      <option value="AmazonPayments" <?php if ($cart->getCheckoutType() == 'AmazonPayments') echo 'selected'; ?>>
        <?php i18n($this->id . '/AMAZONPAYM'); ?>
      </option>
      <option value="GoogleCheckout" <?php if ($cart->getCheckoutType() == 'GoogleCheckout') echo 'selected'; ?>>
	...
	<p>
	  ...
	  <input type="text" class="text" name="shippingflatrate" value="<?php echo $cart->getShippingFlatRate(); ?>"/>
	</p>
	<p>
	  <label for="shippingquantityrate"><?php i18n($this->id . '/QUANTITY_RATE'); ?> : </label>
	  <input type="text" class="text" name="shippingquantityrate" value="<?php echo $cart->getShippingQuantityRate(); ?>"/>
	</p>
	<p>
	  <label for="shippingtotalrate"><?php i18n($this->id . '/TOTAL_RATE'); ?> : </label>
	  <input type="text" class="text" name="shippingtotalrate" value="<?php echo $cart->getShippingTotalRate(); ?>"/>
	</p>
</div>
<div class="clear"></div>
...
<p>
  ...
  <textarea id="cartcss" name="css" style="height: 200px !important;"><?php echo $cart->getCSS(); ?></textarea>
  <?php $textarea = 'cartcss'; $mode = 'text/css'; include('codemirror.php'); ?>
</p>
<p>
  <h4 style="font-weight: bold;"><?php i18n($this->id . '/TEMPLATE'); ?> : </h4>
  <textarea id="carttemplate" name="template" style="height: 200px !important;"><?php echo $cart->getCartTemplate(); ?></textarea>
